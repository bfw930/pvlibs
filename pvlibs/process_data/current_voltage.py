...
def process_current_voltage(_db, _index, _process):
    ...
    Args:
        ...
        _index (int): device_state node index within database instance measurement list
    ...
    # get device data node
    device_state = _db['device_state'][ measurement['rels']['device_state'][0] ]
    device = _db['device'][ measurement['rels']['device'][0] ]


    # for solar performance analysis
    if _process['process'] == 'standard':

        # get 1 sun illumination current-voltage measurement data node
        params = {'measurement_type': 'current_voltage', 'measurement_subtype': 'full',
            'device_id': device['params']['device_id'], 'device_state_id': device_state['params']['device_state_id']}
        index = database.get_index_hard_match_params(_db = _db, _type = 'measurement', _params = params)[0]
        full_data = _db['measurement'][index]['data']

        # get half 1 sun illumination current-voltage measurement data node
        params = {'measurement_type': 'current_voltage', 'measurement_subtype': 'half',
            'device_id': device['params']['device_id'], 'device_state_id': device_state['params']['device_state_id']}
        index = database.get_index_hard_match_params(_db = _db, _type = 'measurement', _params = params)[0]
        half_data = _db['measurement'][index]['data']

        # get dark current-voltage measurement data node
        params = {'measurement_type': 'current_voltage', 'measurement_subtype': 'dark',
            'device_id': device['params']['device_id'], 'device_state_id': device_state['params']['device_state_id']}
        index = database.get_index_hard_match_params(_db = _db, _type = 'measurement', _params = params)[0]
        dark_data = _db['measurement'][index]['data']


        # perform calculations, return results
        results = process_standard(device_state_params = device_state['params'], full_data = full_data,
